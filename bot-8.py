...
def get_user(db, user_id):
    ...
    # পুরনো ইউজারদের ক্ষেত্রে chat_history key না থাকলে যোগ করে দেওয়া হচ্ছে
...
def send_long_answer(chat_id, answer: str, reply_to_message_id=None):
    """
    ...
    চ্যাট মেসেজ হিসেবেই পাঠায়।
    """
    if not should_send_as_file(answer):
...
def photo_handler(message):
    ...
    try:
        ...
        params = {
            active_vision_api["image_param"]: image_url,
            active_vision_api["apikey_param"]: active_vision_api["key"],
            "query": caption_text,
        }
        res = requests.get(active_vision_api["url"], params=params, timeout=60)
...
def general_text_handler(message):
    ...
    # ---- ছবি চাওয়া হয়েছে কিনা চেক করা হচ্ছে, হলে আলাদা ইমেজ API ব্যবহার হবে ----
    if is_image_request(text):
        ...
        try:
            ...
            res = requests.get(active_image_api["url"], params=params, timeout=60)
    ...
    # ---- বড় কোড/লম্বা উত্তর হলে ফাইল আকারে পাঠানো হয়, নাহলে সাধারণ মেসেজ ----
